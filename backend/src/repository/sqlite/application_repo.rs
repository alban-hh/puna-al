...
use crate::repository::sqlite::convert::{
    encode_dt, encode_uuid, parse_dt, parse_enum, parse_uuid,
};
...
impl ApplicationRepository for SqliteApplicationRepository {
    ...
    async fn find_by_id(&self, id: Uuid) -> AppResult<Option<Application>> {
        let row = sqlx::query_as::<_, ApplicationRow>(
            "SELECT id, job_id, applicant_id, cover_letter, cv_url, status, created_at, \
             updated_at FROM applications WHERE id = ?",
        )
        .bind(encode_uuid(id))
    ...
    async fn exists(&self, job_id: Uuid, applicant_id: Uuid) -> AppResult<bool> {
        let found: i64 = sqlx::query_scalar(
            "SELECT EXISTS(SELECT 1 FROM applications WHERE job_id = ? AND applicant_id = ?)",
        )
    ...
    async fn list_by_job(
        ...
        let total: i64 = sqlx::query_scalar("SELECT COUNT(*) FROM applications WHERE job_id = ?")
            .bind(encode_uuid(job_id))
            .fetch_one(&self.pool)
            .await?;
        ...
        let rows = sqlx::query_as::<_, JobApplicationRow>(
            "SELECT a.id, a.job_id, a.applicant_id, a.cover_letter, a.cv_url, a.status, \
             ...
             WHERE a.job_id = ? ORDER BY a.created_at DESC LIMIT ? OFFSET ?",
        )
        .bind(encode_uuid(job_id))
        .bind(page.limit)
        .bind(page.offset)
    ...
    async fn list_by_applicant(
        ...
        let total: i64 =
            sqlx::query_scalar("SELECT COUNT(*) FROM applications WHERE applicant_id = ?")
                .bind(encode_uuid(applicant_id))
                .fetch_one(&self.pool)
                .await?;
        ...
        let rows = sqlx::query_as::<_, ApplicantApplicationRow>(
            "SELECT a.id, a.job_id, a.applicant_id, a.cover_letter, a.cv_url, a.status, \
             ...
             WHERE a.applicant_id = ? ORDER BY a.created_at DESC LIMIT ? OFFSET ?",
        )
        .bind(encode_uuid(applicant_id))
        .bind(page.limit)
        .bind(page.offset)
