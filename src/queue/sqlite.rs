...
impl SqliteQueue {
    pub fn new(pool: sqlx::SqlitePool, default_max_attempts: i64) -> Self {
        Self {
            pool,
...
impl Queue for SqliteQueue {
    ...
    async fn claim_due(&self, now: DateTime<Utc>, limit: i64) -> AppResult<Vec<QueueJob>> {
        let rows = sqlx::query_as::<_, QueueJobRow>(&format!(
            "UPDATE queue_jobs SET status = 'processing', attempts = attempts + 1, updated_at = ? \
             WHERE id IN ( \
                 SELECT id FROM queue_jobs WHERE status = 'pending' AND run_at <= ? \
    ...
    async fn mark_sent(&self, id: Uuid, completed_at: DateTime<Utc>) -> AppResult<()> {
        sqlx::query("UPDATE queue_jobs SET status = 'sent', updated_at = ? WHERE id = ?")
    ...
    async fn reschedule(&self, id: Uuid, run_at: DateTime<Utc>, last_error: &str) -> AppResult<()> {
        sqlx::query(
            "UPDATE queue_jobs SET status = 'pending', run_at = ?, last_error = ?, \
             updated_at = ? WHERE id = ?",
        )
        .bind(encode_dt(run_at))
        .bind(last_error)
